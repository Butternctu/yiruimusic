                    ...
                    <div className="md:hidden space-y-4">
                      {filteredMembers.map(member => {
                        ...
                        if (isEditing) {
                          return (
                            <div
                              key={`edit-mobile-${member.id}`}
                              className="glass-card p-6 rounded-sm border border-gold/30 bg-dark-800 shadow-2xl animate-fadeInUp"
                            >
                          ...
                          <div key={member.id} className="glass-card p-5 rounded-sm border border-white/3 space-y-5">
                            <div className="flex items-start justify-between">
                              ...
                              <div className="flex flex-col items-end space-y-1">
                                <span
                                  className={`inline-block px-2 py-0.5 text-[8px] uppercase tracking-[0.2em] rounded-sm border ${tier.borderColor} ${tier.textColor} ${tier.bgColor}`}
                                >
                                  {tier.name}
                                </span>
                    ...
                    <div className="relative z-10">
                      ...
                      <form onSubmit={handleCreateUser} className="space-y-6" autoComplete="off" noValidate>
                        ...
                        {createResult.message && (
                          <div
                            className={`mt-2 ${createResult.type === 'error' ? (shakeError ? 'animate-error-shake' : 'animate-error-pulse') : 'animate-fadeIn'}`}
                          >
                            <p
                              className={`text-[11px] tracking-widest uppercase text-center font-medium ${createResult.type === 'error' ? 'text-[#d9736c]' : 'text-gold'}`}
                            >
                              {createResult.message}
                            </p>
                          </div>
